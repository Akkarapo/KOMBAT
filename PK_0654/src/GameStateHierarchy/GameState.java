...
import com.example.demo.PK_0654.src.Utilities.*;
    ...
    GameState(int mode, int spawn_cost,int hex_purchase_cost,long init_budget,int init_hp,int turn_budget,int max_budget,int interest_pct,int max_turn,int max_spawns, int minionTypeAmount , String[] minionName, int[] minionDEF, String[] MinionStrategy) {
       ...
       //set row and col แต่ละช่อง
        ...
        for (int i = 0; i < this.minionTypeAmount; i++) {
            MinionType[i] = new Minion("000",minionName[i],config.getInitHP(),minionDEF[i],MinionStrategy[i],i);
        }
    ...
    int GameStart(){
        while(nowTurn<=config.getMaxTurns()){
            ...
            if(currentPlayer.getPlayerType() == "Bot"){
                ...
                while(playing) {
                    ...
                    //End Turn
                        ...
                        targetHex = getRowAndCol("Dest");
                        int[] nowHex = getRowAndCol("Now");;
                        moveMinion(targetHex,nowHex);
                    }
                    else if(playerChoice == 5){
                        targetHex = getRowAndCol("find ally from");
                        findNearestMinionDistance(targetHex,currentPlayer.getPlayerNumber());
                    }
                    else if(playerChoice == 6){
                        targetHex = getRowAndCol("find ally from");
                        String target = currentPlayer.getPlayerNumber() == "001" ? "002":"001";
                        findNearestMinionDistance(targetHex,target);
                    }
                    else if(playerChoice == 7){
                        targetHex = getRowAndCol("target");
                        int[] nowHex = getRowAndCol("Attacker");
                        System.out.print("Spend for damage : ");
                        int butgetATK = Choice.nextInt();
                        attackEnemy(targetHex,nowHex,butgetATK);
                    }
            ...
            for (Map.Entry<Integer, int[]> entry : minions.entrySet()) {
                int type = gameBoard[entry.getValue()[0]][entry.getValue()[1]].getMinion().getMinionType()+1;
                if(gameBoard[entry.getValue()[0]][entry.getValue()[1]].hasMinion()){ minionBot(type,new int[]{entry.getValue()[0],entry.getValue()[1]});}
            }*/
...
/*
    private void minionBot(int type,int[] minionNowHex){
        type = 1 ;

        int[]   targetHex,tempHex = new int[]{-1,-1},allyHex;
        int     cost,temp,count,ran,digitOfHP,digitOfDEF,max,min;
        String  Enemy = currentPlayer.getPlayerNumber() =="001" ? "002" : "001" ;
        boolean x = true;

        targetHex   = findNearestMinionDistance(minionNowHex,Enemy);
        allyHex     = findNearestMinionDistance(minionNowHex,currentPlayer.getPlayerNumber());

        if(type == 1){
            while(x){
                if(currentPlayer.getBudget() < config.getTurnBudget()) x = false;
                else if(targetHex[0] == -1){
                    ran =  new Random().nextInt(6)+1;
                    if(ran == 1)        tempHex = up(minionNowHex[0],minionNowHex[1]);
                    else if(ran == 2)   tempHex = upright(minionNowHex[0],minionNowHex[1]);
                    else if(ran == 3)   tempHex = downright(minionNowHex[0],minionNowHex[1]);
                    else if(ran == 4)   tempHex = down(minionNowHex[0],minionNowHex[1]);
                    else if(ran == 5)   tempHex = downleft(minionNowHex[0],minionNowHex[1]);
                    else if(ran == 6)   tempHex = upleft(minionNowHex[0],minionNowHex[1]);

                    if(tempHex[0] == -1 || gameBoard[tempHex[0]][tempHex[1]].hasMinion())    x = false;
                    else if(tempHex[0]!= -1 && minionNowHex[0] == -1){
                        moveMinion(tempHex,minionNowHex);
                        minionNowHex = tempHex;
                        tempHex = new int[]{-1,-1};
                    }
                }
                else if(targetHex[0] != -1 && allyHex[0] == -1){
                    if(minionNowHex[1] == targetHex[1]){
                        if(minionNowHex[0] > targetHex[0]){
                            while(true){
                                tempHex = up(minionNowHex[0],minionNowHex[1]);
                                if(tempHex == targetHex) break;
                                else if(tempHex[0]!= -1 && minionNowHex[0] == -1){
                                    moveMinion(tempHex,minionNowHex);
                                    minionNowHex = tempHex;
                                }
                            ...
                            temp = nearbyUp(minionNowHex,Enemy);
                            digitOfHP = (int) temp /100;
                            digitOfDEF = ((int)(temp / 10)) % 10;

                            max = ((int) Math.pow(10, digitOfHP) - 1) + ((int) Math.pow(10, digitOfDEF) - 1) ;
                            min = ((int) Math.pow(10, digitOfDEF) - 1);
                            if(currentPlayer.getBudget() < max + min) x = false ;
                            else{
                                cost =  new Random().nextInt(max)+min;
                                attackEnemy(targetHex,minionNowHex,cost);
                            }

                        }else{
                            while(true){
                                tempHex = down(minionNowHex[0],minionNowHex[1]);
                                if(tempHex == targetHex) break;
                                else if(tempHex[0]!= -1 && minionNowHex[0] == -1){
                                    moveMinion(tempHex,minionNowHex);
                                    minionNowHex = tempHex;
    ...
    * */
    ...
    private void moveMinion(int[] targetHex, int[] nowHexCoor) {
        // ตรวจสอบว่าค่าอยู่ในขอบเขตของกระดาน
        if (targetHex[0] < 0 || targetHex[0] >= 8 || targetHex[1] < 0 || targetHex[1] >= 8) {
            System.err.println("moveMinion: Target position is out of bounds (" + targetHex[0] + ", " + targetHex[1] + ")");
            return;
        }
        if (nowHexCoor[0] < 0 || nowHexCoor[0] >= 8 || nowHexCoor[1] < 0 || nowHexCoor[1] >= 8) {
            System.err.println("moveMinion: Current position is out of bounds (" + nowHexCoor[0] + ", " + nowHexCoor[1] + ")");
            return;
        }

        // หักงบประมาณของผู้เล่น
        currentPlayer.useBudget(1);

        // ตรวจสอบว่ามีมินเนี่ยนอยู่ที่ตำแหน่งเป้าหมายหรือไม่
        if (gameBoard[targetHex[0]][targetHex[1]].hasMinion()) {
            System.out.println("Already have minion in target hex");
        }
        // ตรวจสอบว่าตำแหน่งปัจจุบันไม่มีมินเนี่ยน หรือไม่ใช่มินเนี่ยนของผู้เล่นนี้
        else if (!gameBoard[nowHexCoor[0]][nowHexCoor[1]].hasMinion() ||
                gameBoard[nowHexCoor[0]][nowHexCoor[1]].getMinion().getOwnerName() != currentPlayer.getPlayerNumber()) {
            System.out.println("Don't have your minion to move");
        }
        // ตรวจสอบว่าตำแหน่งที่ไปไกลเกิน 1 ช่องหรือไม่
        else if (Math.abs(targetHex[0] - nowHexCoor[0]) > 1 || Math.abs(targetHex[1] - nowHexCoor[1]) > 1) {
            System.out.println("\nThis hex is too far\n");
        }
        // ทำการย้ายมินเนี่ยน
        else {
            ...
            System.out.println("Minion moved from (" + nowHexCoor[0] + ", " + nowHexCoor[1] + ") to (" + targetHex[0] + ", " + targetHex[1] + ")");
        }
    }
    ...
    private int[] findNearestMinionDistance(int[] targetHex,String target){
        ...
        if(targetHex[1] != 7 || !(targetHex[0] == 7 && (targetHex[1] % 2 == 0) ) ){
            ...
            int col = targetHex[1]+1;
            while(row < 8 && col < 8){ // ป้องกัน out of bounds
                if(gameBoard[row][col].hasMinion() && gameBoard[row][col].getMinion().getOwnerName().equals(target)){
                    if(col - targetHex[1] < distance){
                        nearestMinionHex = new int[]{row,col};
                        distance = col - targetHex[1];
...
//down
        if(targetHex[0] != 7){
            for(int i = targetHex[0]+1; i < 8; i++){ // ป้องกัน out of bounds
                if(gameBoard[i][targetHex[1]].hasMinion() && gameBoard[i][targetHex[1]].getMinion().getOwnerName().equals(target)){
                    nearestMinionHex = new int[]{i, targetHex[1]};
...
//downleft
        if(targetHex[1] != 0 || !(targetHex[0] == 7 && (targetHex[1] % 2 == 0) ) ){
            int row = targetHex[1] % 2 == 0 ? targetHex[0]+1 : targetHex[0];
            int col = targetHex[1]-1;
            while(row < 8 && col >= 0){ // ป้องกัน out of bounds
                if(gameBoard[row][col].hasMinion() && gameBoard[row][col].getMinion().getOwnerName().equals(target)){
                    if(targetHex[1] - col < distance){
                        nearestMinionHex = new int[]{row,col};
...
//upleft
        if(targetHex[1] != 0 || !(targetHex[0] == 0 && (targetHex[1] % 2 == 1) ) ){
            int row = targetHex[1] % 2 == 0 ? targetHex[0] : targetHex[0] - 1;
            int col = targetHex[1]-1;
            while(row >= 0 && col >= 0){ // ป้องกัน out of bounds
                if(gameBoard[row][col].hasMinion() && gameBoard[row][col].getMinion().getOwnerName().equals(target)){
                    if(col - targetHex[1] < distance){
                        nearestMinionHex = new int[]{row,col};
                        distance = col - targetHex[1];
